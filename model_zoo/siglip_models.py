    ...
    def get_retrieval_scores_batched(self, joint_loader):
        ...
        for batch in tqdm_loader:
            ...
            for i_option in batch["image_options"]:
                ...
                pixel_values = i_option['pixel_values']
                pixel_values = pixel_values.squeeze(1).to(self.device)
                image_embeddings = self.model.get_image_features(pixel_values=pixel_values).cpu().numpy() # B x D
            ...
            for c_option in batch["caption_options"]:
                ...
                caption_tokenized = self.preprocessor(c_option, return_tensors="pt", padding=True).to(self.device)['input_ids']
                # caption_embeddings = self.model.encode_text(caption_tokenized.to(self.device)).cpu().numpy() # B x D
                caption_embeddings = self.model.get_text_features(caption_tokenized).cpu().numpy()
